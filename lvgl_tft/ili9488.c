...
void ili9488_init(void)
{
	lcd_init_cmd_t ili_init_cmds[]={
		...
		{ILI9488_CMD_MEMORY_ACCESS_CONTROL, {(1<<7)|(1<<6)|(1<<5)|(0<<3)}, 1},
		...
		{ILI9488_CMD_DISP_INVERSION_ON, {0x00}, 0},
				    {0x2b, {0x00, 0x00, 0x01, 0X40}, 4},
...
void ili9488_flush(lv_disp_drv_t * drv, const lv_area_t * area, lv_color_t * color_map)
{
	...
	// ESP_LOGI(TAG, "x_start:%d x_end:%d y_start:%d y_end%d", area->x1, area->x2, area->y1, area->y2);
...
// ESP_LOGI(TAG, "sppend111::%lld",  end_time - start_time);
	/*Column addresses*/
	ili9488_send_cmd(ILI9488_CMD_COLUMN_ADDRESS_SET);
	ili9488_send_data(xb, 4);
// end_time = esp_timer_get_time();
// ESP_LOGI(TAG, "sppend222::%lld",  end_time - start_time);
	/*Page addresses*/
	ili9488_send_cmd(ILI9488_CMD_PAGE_ADDRESS_SET);
	ili9488_send_data(yb, 4);
// end_time = esp_timer_get_time();
// ESP_LOGI(TAG, "sppend333::%lld",  end_time - start_time);
	/*Memory write*/
	ili9488_send_cmd(ILI9488_CMD_MEMORY_WRITE);

	ili9488_send_color((void *) mybuf, size * 3);
// 	end_time = esp_timer_get_time();
// ESP_LOGI(TAG, "sppend444::%lld",  end_time - start_time);
	heap_caps_free(mybuf);
// 	end_time = esp_timer_get_time();
// ESP_LOGI(TAG, "sppend555::%lld",  end_time - start_time);
}
